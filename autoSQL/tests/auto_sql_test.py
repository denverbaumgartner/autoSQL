...
class TestSQLData:
    ...
    def test_data_generator(self):
        ...
        assert isinstance(sd.data_generator.generate_random_data(data_type="VARCHAR"), str)
        ...
        filler_data = sd.data_generator.generate_filler_data(column_types=columns, num_records=1)
    ...
    def test_data_transformations(self): 
        ...
        assert test_set['train'][80]['answer'] == "SELECT T1.name, T1.id FROM station AS T1 JOIN status AS T2 ON T1.id = T2.station_id GROUP BY T2.station_id HAVING AVG(T2.bikes_available) > 14 UNION SELECT name, id FROM station WHERE installation_date LIKE '12/%'"
        ...
        assert test_set['train'][80]['column_types'] == '{"station": {"name": "VARCHAR", "id": "VARCHAR", "installation_date": "VARCHAR"}, "status": {"station_id": "VARCHAR", "bikes_available": "INT"}}'
        ...
        assert list(json.loads(test_set['train'][80]['filler_data']).keys()) == ['station', 'status']
        # TODO: we should expand the test set here to include check that types are properly generated
    ...
    def test_data_filters(self): 
        ...
        assert len(test_set['train']) < 90 # TODO: because we are randomly generating sample data, this value is not deterministic. we should have a better test here
